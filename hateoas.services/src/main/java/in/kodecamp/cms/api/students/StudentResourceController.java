...
public class StudentResourceController
    extends BaseResourceController<StudentResource> {
  ...
  public CollectionResource list(@Context UriInfo info,
    ...
    System.out.println("isExpanded : " + isExpanded + ", offset: " + offset
        + ", size: " + size);
    List<StudentEntity> resultList = studentBo.getAllActive();
    return super.createCollectionResource(info, PathConstants.PATH_STUDENTS,
        resultList, isExpanded);
  }

  @GET
  @Path("/{id}")
  @RolesAllowed("STUDENT")
  public StudentResource getBy(@Context UriInfo uriInfo,
  ...
  public Response update(@Context UriInfo uriInfo, @PathParam("id") long id,
    ...
    System.out.println(
        this.className + "update at id : " + id + " with -> " + fieldValueMap);
    ...
    return Response.ok(new StudentResource(uriInfo, foundEntity))
        .build();
  }

  @POST
  public Response create(final @Context UriInfo uriInfo,
      final StudentEntity newStudent) {
  ...
  public Response deleteBy(@PathParam("id") final long id) {
    ...
    return Response.ok("Record Deleted Successfully")
        .build();
  }
